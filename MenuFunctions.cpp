...
/* Interrupt driven periodic handler */
  ...
  int MenuFunctions::updateTouch(int x, int y, int threshold) {
    if (!display_obj.headless_mode) {
      // return display_obj.tft.getTouch(x, y, threshold);

      if (display_obj.isTouched){
          x = display_obj.touchX;
          y = display_obj.touchY;
          // threshold = display_obj.tp.points[0].size;
      }
  ...
  bool my_touchpad_read(lv_indev_drv_t * indev_driver, lv_indev_data_t * data)
  {
    ...
    if (display_obj.isTouched){
      touchX = display_obj.touchX;
      touchY = display_obj.touchY;
    }
    ...
    else
    {
      ...
      data->state = display_obj.isTouched ? LV_INDEV_STATE_PR : LV_INDEV_STATE_REL; 
...
void MenuFunctions::main(uint32_t currentTime)
{
  ...
  // Touched will be set true is there is a valid touch on the screen
  ...
  // This is for when on a menu
        ...
        (wifi_scan_obj.currentScanMode != WIFI_ATTACK_RICK_ROLL))
    {
      ...
      for (uint8_t b = 0; b < BUTTON_ARRAY_LEN; b++) {
        if (touched && display_obj.key[b].contains(t_x, t_y)) {
          display_obj.key[b].press(true);  // tell the button it is touched
        } else {
          display_obj.key[b].press(false);  // tell the button it is NOT touched
        }
      }
...
{
  Serial.println("Setup menu functions");
  extern LinkedList<AccessPoint>* access_points;
  extern LinkedList<Station>* stations;
  ...
  #if (!defined(HAS_ST7796) && defined(HAS_BUTTONS))
...
void MenuFunctions::buildButtons(Menu * menu, int starting_index, String button_name)
{
  if (menu->list != NULL)
  {
    ...
    for (uint8_t i = 0; i < menu->list->size(); i++)
    {
      ...
      display_obj.key[i].setLabelDatum(BUTTON_PADDING + MENU_BUTTON_SPACING_LEFT - (KEY_W / 2), 2, ML_DATUM);
    }
  }
}
...
void MenuFunctions::displayCurrentMenu(uint8_t start_index)
{
  ...
  Serial.println("Displaying current menu index"+((String)start_index));
  display_obj.clearScreen();
  display_obj.updateBanner(current_menu->name);
