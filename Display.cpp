...
void Display::RunSetup()
{
  ...
  tft.setCursor(0, 0);
    ...
    #ifdef TFT_SHIELD
      uint16_t calData[5] = CALIBRATION_PORTRAIT; // tft.setRotation(0); // Portrait with TFT Shield
      Serial.println(F("Using TFT Shield"));
    #else if defined(TFT_DIY)
      uint16_t calData[5] = CALIBRATION_PORTRAIT; // tft.setRotation(0); // Portrait with DIY TFT
...
void Display::tftDrawGraphObjects(byte x_scale)
{
  ...
  tft.drawFastVLine(10, 5, 320, TFT_WHITE); // y axis
  ...
  tft.setCursor(3, 320); tft.print("0"); // "-" at bottom of y axis
}
...
void Display::tftDrawXScaleButtons(byte x_scale)
{
  ...
  tft.setCursor(208, 430); tft.setTextColor(TFT_WHITE); tft.setTextSize(1); tft.print("X Scale:"); tft.print(x_scale);

  key[0].initButton(&tft, // x - box
                        220,
                        400, // x, y, w, h, outline, fill, text
                        20,
                        20,
                        TFT_BLACK, // Outline
                        TFT_CYAN, // Fill
...
void Display::tftDrawYScaleButtons(byte y_scale)
{
  ...
  tft.setCursor(265, 430); tft.setTextColor(TFT_WHITE); tft.setTextSize(1); tft.print("Y Scale:"); tft.print(y_scale);
...
void Display::tftDrawChannelScaleButtons(int set_channel)
{
  ...
  tft.setCursor(145, 430); tft.setTextColor(TFT_WHITE); tft.setTextSize(1); tft.print(text10); tft.print(set_channel);
...
{
  Serial.println("TOUCH TOEXIT CALLED");
...
{  
  ...
  if (this->tp.isTouched) {
     Serial.print("Touched x: ");Serial.print(this->touchX);Serial.print(", y: ");Serial.print(this->touchY);Serial.println(" ");
  }

  if (this->touchTime + 10 < millis()) {
    if (this->tp.isTouched){
      if (this->isTouched == false && this->touchX != this->tp.points[0].x && this->touchY != this->tp.points[0].y) {
        this->isTouched = true;
        this->touchX = this->tp.points[0].x;
        this->touchY = this->tp.points[0].y;
        Serial.print("Touch ");Serial.print(1);Serial.print(": ");
        Serial.print("  x: ");Serial.print(this->touchX);
        Serial.print("  y: ");Serial.print(this->touchY);
