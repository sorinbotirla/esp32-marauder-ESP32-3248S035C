...
void WiFiScan::startWiFiAttacks(uint8_t scan_mode, uint16_t color, String title_string) {
  ...
  this->wifi_initialized = true;
    ...
    led_obj.setMode(MODE_ATTACK_WIFI);
  #endif
  initTime = millis();
}
...
void WiFiScan::RunEvilPortal(uint8_t scan_mode, uint16_t color)
{
  ...
  #ifdef HAS_NEOPIXEL_LED
    led_obj.setMode(MODE_SERVE_WIFI);
  #endif
...
void WiFiScan::RunAPScan(uint8_t scan_mode, uint16_t color)
{
  ...
  #ifdef HAS_NEOPIXEL_LED
    led_obj.setMode(MODE_SCAN_WIFI);
  #endif
  ...
  void WiFiScan::RunLvJoinWiFi(uint8_t scan_mode, uint16_t color) {
    ...
    #ifdef TFT_SHIELD
      uint16_t calData[5] = CALIBRATION_LANDSCAPE; // Landscape TFT Shield
      // Serial.println("Using TFT Shield");
    #else if defined(TFT_DIY)
      uint16_t calData[5] = CALIBRATION_LANDSCAPE; // Landscape TFT DIY
      // Serial.println("Using TFT DIY");
    #endif
    #if (defined(HAS_ST7796))
      // display_obj.tft.setTouch(calData);
      // display_obj.tp.setRotation(ROTATION_RIGHT); 
      // display_obj.tp.setResolution(TOUCH_HEIGHT,TOUCH_WIDTH);

      display_obj.tp.setRotation(ROTATION_INVERTED); 
      display_obj.tp.setResolution(TOUCH_WIDTH,TOUCH_HEIGHT);
      // Serial.println("TOUCH SET ROTATION_RIGHT 1");
    #endif
...
void WiFiScan::RunGPSInfo() {
    ...
    #ifdef HAS_NEOPIXEL_LED
      if (gps_obj.getFixStatus()){
        led_obj.setMode(MODE_GPS_FIX);
      } else {
        led_obj.setMode(MODE_GPS_NOFIX);
      }
    #endif
  #endif
}
...
{
  #ifdef HAS_NEOPIXEL_LED
    led_obj.setMode(MODE_SNIFF_WIFI);
  #endif
  ...
  startPcap("packet_monitor");
      ...
      display_obj.tft.fillScreen(TFT_BLACK);
        ...
        // display_obj.tp.setRotation(ROTATION_RIGHT); 
        // display_obj.tp.setResolution(TOUCH_HEIGHT,TOUCH_WIDTH);
        // Serial.println("TOUCH SET ROTATION_RIGHT 2");
...
void WiFiScan::RunEapolScan(uint8_t scan_mode, uint16_t color)
{
  ...
  num_eapol = 0;
    ...
    startPcap("eapol");
        ...
        // display_obj.tp.setRotation(ROTATION_RIGHT); 
        // display_obj.tp.setResolution(TOUCH_HEIGHT,TOUCH_WIDTH);
        // Serial.println("TOUCH SET ROTATION_RIGHT 3");
      ...
      display_obj.tft.setFreeFont(NULL);
      display_obj.tft.setTextSize(1);
      display_obj.tft.fillRect(127, 400, 193, 28, TFT_BLACK); // Buttons
      display_obj.tft.fillRect(12, 400, 90, 32, TFT_BLACK); // color key
...
void WiFiScan::executeSourApple() {
    ...
    #ifdef HAS_NEOPIXEL_LED
      led_obj.setMode(MODE_ATTACK_BT);
    #endif
  #endif
}
...
void WiFiScan::RunRawScan(uint8_t scan_mode, uint16_t color)
{
  if (scan_mode != WIFI_SCAN_SIG_STREN)
    ...
    display_obj.tft.setTextColor(TFT_WHITE, color);
      ...
      display_obj.touchToExit();
      Serial.println("TOUCH TOEXIT 10");
    #endif
    display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
    display_obj.setupScrollArea(display_obj.TOP_FIXED_AREA_2, BOT_FIXED_AREA);
  #endif
...
void WiFiScan::RunDeauthScan(uint8_t scan_mode, uint16_t color)
{
  startPcap("deauth");
    ...
    display_obj.tft.setTextColor(TFT_BLACK, color);
      ...
      display_obj.touchToExit();
      Serial.println("TOUCH TOEXIT 11");
    #endif
...
void WiFiScan::RunProbeScan(uint8_t scan_mode, uint16_t color)
{
  if (scan_mode == WIFI_SCAN_PROBE)
    ...
    display_obj.tft.setTextColor(TFT_BLACK, color);
      ...
      display_obj.touchToExit();
      Serial.println("TOUCH TOEXIT 12");
    #endif
    display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
    display_obj.setupScrollArea(display_obj.TOP_FIXED_AREA_2, BOT_FIXED_AREA);
  #endif
...
void WiFiScan::RunSourApple(uint8_t scan_mode, uint16_t color) {
    ...
    pAdvertising = pServer->getAdvertising();*/
      ...
      display_obj.tft.setTextColor(TFT_BLACK, color);
        ...
        display_obj.touchToExit();
        Serial.println("TOUCH TOEXIT 13");
      #endif
      display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
    #endif
...
void WiFiScan::RunSwiftpairSpam(uint8_t scan_mode, uint16_t color) {
      ...
      display_obj.tft.setTextColor(TFT_BLACK, color);
        ...
        display_obj.touchToExit();
        Serial.println("TOUCH TOEXIT 14");
      #endif
      display_obj.tft.setTextColor(TFT_GREEN, TFT_BLACK);
    #endif
...
void WiFiScan::RunBluetoothScan(uint8_t scan_mode, uint16_t color)
{
    ...
    if (scan_mode == BT_SCAN_ALL)
    {
        ...
        display_obj.tft.setTextColor(TFT_BLACK, color);
          ...
          display_obj.touchToExit();
          Serial.println("TOUCH TOEXIT 15");
        #endif
      ...
      pBLEScan->setAdvertisedDeviceCallbacks(new bluetoothScanAllCallback(), false);
        ...
        display_obj.tft.setTextColor(TFT_BLACK, color);
          ...
          display_obj.touchToExit();
          Serial.println("TOUCH TOEXIT 16");
        #endif
        display_obj.tft.setTextColor(TFT_CYAN, TFT_BLACK);
        display_obj.setupScrollArea(display_obj.TOP_FIXED_AREA_2, BOT_FIXED_AREA);
      #endif
...
void WiFiScan::activeEapolSnifferCallback(void* buf, wifi_promiscuous_pkt_type_t type)
{
  ...
  //  for (x_pos = (11 + x_scale); x_pos <= 320; x_pos += x_scale) //go along every point on the x axis and do something, start over when finished
    for (x_pos = (11 + x_scale); x_pos <= TFT_HEIGHT; x_pos = x_pos)
    {
      ...
      // int t_x = 0, t_y = 0; // To store the touch coordinates
  
      // Do the touch stuff
      #if (defined(HAS_ST7796))
        display_obj.tp.read();

        // if (display_obj.tp.isTouched) {
        //   Serial.print("Touched x: ");Serial.print(display_obj.touchX);Serial.print(", y: ");Serial.print(display_obj.touchY);Serial.println(" ");
        // }

        if (display_obj.touchTime + 10 < millis()) {
          if (display_obj.tp.isTouched){
            if (display_obj.isTouched == false && display_obj.touchX != display_obj.tp.points[0].x && display_obj.touchY != display_obj.tp.points[0].y) {
              display_obj.isTouched = true;
              display_obj.touchX = display_obj.tp.points[0].x;
              display_obj.touchY = display_obj.tp.points[0].y;
              // Serial.print("Touch ");Serial.print(1);Serial.print(": ");
              // Serial.print("  x: ");Serial.print(display_obj.touchX);
              // Serial.print("  y: ");Serial.print(display_obj.touchY);
              // // Serial.print("  size: ");Serial.println(tp.points[0].size);
              // Serial.println(' ');
              display_obj.touchTime = millis();
              // this->isTouched = false; 
            } else {
              // display_obj.isTouched = false;
            }
          } else {
            display_obj.isTouched = false;
          }
        }
        // touched = display_obj.isTouched; // TBA FIX THIS, IS NOT GETTING ANY TOUCH 
      #endif
  
      // if (touched) {
      //   t_x = display_obj.touchX;
      //   t_y = display_obj.touchY;
      ...
      {
        if (display_obj.isTouched && display_obj.key[b].contains(display_obj.touchX, display_obj.touchY))
        {
        ...
        if (display_obj.key[b].justReleased())
        {
          ...
          if (b == 4) {
            if (set_channel > 1) {
              ...
              set_channel--;
              delay(70);
              display_obj.tft.fillRect(127, 430, 193, 28, TFT_BLACK);
              display_obj.tftDrawChannelScaleButtons(set_channel);
              display_obj.tftDrawExitScaleButtons();
              changeChannel();
              break;
      ...
      if (currentTime - initTime >= (GRAPH_REFRESH * 5)) {
        ...
        else
        {
          ...
          display_obj.tft.fillRect(x_pos+1, 121, 10, 198, TFT_BLACK);
        }
...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  else if ((currentScanMode == WIFI_ATTACK_RICK_ROLL))
  {
    ...
    if (currentTime - initTime >= 1000)
    {
      ...
      #ifdef TFT_SHIELD
        uint16_t calData[5] = CALIBRATION_PORTRAIT; // Landscape TFT Shield
        // Serial.println("Using TFT Shield");
      #else if defined(TFT_DIY)
        uint16_t calData[5] = CALIBRATION_PORTRAIT; // Landscape TFT DIY
        // Serial.println("Using TFT DIY");
      #endif
      #if (defined(HAS_ST7796))
        // display_obj.tft.setTouch(calData);
        // Serial.println("TOUCH SET ROTATION_INVERTED 1");
        display_obj.tp.setRotation(ROTATION_INVERTED);  
        display_obj.tp.setResolution(TOUCH_WIDTH,TOUCH_HEIGHT);
      #endif
    #endif
